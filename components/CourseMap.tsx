...
function mapPlaces(course: Course): MapPlace[] {
  ...
  const waypointPlaces = course.waypoints
    .filter(hasCoordinate)
    .map((place, index) => ({ place, kind: "waypoint" as const, order: index + 1 }));
...
function markerHtml(item: MapPlace) {
  ...
  const size = isWaypoint ? 30 : 38;
  const color = isWaypoint ? "#7c3aed" : "#ff5c8a";
  const label = String(item.order);

  return `
    <div style="
      width:${size}px;height:${size}px;border-radius:999px;background:${color};color:white;
      display:grid;place-items:center;font-size:${isWaypoint ? 13 : 16}px;font-weight:900;
...
function labelHtml(item: MapPlace) {
  const safeName = escapeHtml(item.place.name);
  const isWaypoint = item.kind === "waypoint";
  const color = isWaypoint ? "#7c3aed" : "#ff5c8a";
  const label = isWaypoint ? `경유 ${item.order}` : `코스 ${item.order}`;

  return `
    <div style="
      transform:translateY(12px);background:white;border:1px solid #ffe4ec;border-radius:14px;
      padding:7px 10px;font-size:12px;font-weight:800;color:#2f2630;
      box-shadow:0 8px 24px rgba(31,23,31,.12);white-space:nowrap;
    ">
      <span style="color:${color};font-weight:900;">${label}</span> · ${safeName}
    </div>
  `;
}
  ...
  return (
    <section className="overflow-hidden rounded-[1.75rem] border border-rose-100 bg-white shadow-card">
      <div className="flex items-start justify-between px-4 py-4">
        <div>
          <h2 className="text-base font-black text-ink">코스 지도</h2>
          <p className="mt-1 text-xs leading-5 text-zinc-500">분홍색은 선택 코스, 보라색 번호는 주변 중간경유지예요.</p>
        </div>
      ...
      <div className="relative border-y border-rose-50">
        ...
        {!isLoaded && <div className="absolute inset-0 grid place-items-center bg-white/70 text-sm font-semibold text-zinc-500">지도를 불러오는 중이에요</div>}
      </div>

      <ol className="space-y-2 p-4 text-sm">
        {visibleItems.map((item) => (
          <li key={`${item.place.id}-${item.kind}-${item.order}`} className="flex items-center gap-3">
            <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-xs font-black text-white ${item.kind === "course" ? "bg-roseApp" : "bg-violet-600"}`}>{item.order}</span>
            <div className="min-w-0">
              <p className="truncate font-semibold text-ink">{placeListLabel(item)}</p>
              <p className="text-xs text-zinc-500">{item.kind === "course" ? "선택한 코스 장소" : "지도 보라색 번호와 같은 중간경유지"}</p>
            </div>
          </li>
        ))}
      </ol>
...
function placeListLabel(item: MapPlace) {
  if (item.kind === "waypoint") return `경유 ${item.order}. ${item.place.name}`;
  return `코스 ${item.order}. ${item.place.name}`;
}
